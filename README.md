...
flowchart TB
    ...
    subgraph Edge
        GW[api-gateway<br/>REST :8080]
    end

    subgraph Write Path
        US[upload-service<br/>gRPC :8081]
    end

    subgraph Orchestration
        SO[saga-orchestrator<br/>gRPC :8082]
    end

    subgraph Workers
        TS[transcoding-service<br/>gRPC :8083]
        TH[thumbnail-service<br/>gRPC :8084]
        MS[metadata-service<br/>gRPC :8085]
    end

    subgraph Read Path
        MQ[media-query-service<br/>REST :8086]
    end

    NS[notification-service<br/>consumer only]
    ...
    K -->|media.uploaded| SO
    K -->|saga.cmd.transcode| TS
    K -->|saga.cmd.thumbnail| TH
    K -->|saga.cmd.metadata| MS
    K -->|saga.reply| SO
    K -->|media.processing.*| NS
    K -->|projections| MQ
...
sequenceDiagram
    ...
    participant MQ as media-query-service
    participant NS as notification-service

    C->>GW: POST /v1/uploads/presign
    GW->>US: gRPC CreatePresignedUpload
    US-->>GW: presigned URL
    GW-->>C: { uploadId, url }

    C->>S3: PUT object (direct upload)

    C->>GW: POST /v1/uploads/{uploadId}/complete
    GW->>US: gRPC ConfirmUpload
    US->>US: TX: update upload + insert media + outbox
    US-->>GW: { mediaId, sagaId, status: PROCESSING }
    GW-->>C: 202 Accepted

    US--)K: media.uploaded.v1 (via outbox relay)
    K--)SO: consume media.uploaded.v1
    SO->>SO: create saga (PENDING → RUNNING)

    SO--)K: saga.cmd.metadata.v1
    K--)MS: consume
    MS->>MS: extract metadata
    MS--)K: saga.reply.v1 (success)

    K--)SO: consume reply
    SO--)K: saga.cmd.transcode.v1
    K--)TS: consume
    TS->>S3: read source, write HLS variants
    TS--)K: saga.reply.v1 (success)

    K--)SO: consume reply
    SO--)K: saga.cmd.thumbnail.v1
    K--)TH: consume
    TH->>S3: read source, write thumbnails
    TH--)K: saga.reply.v1 (success)

    K--)SO: consume reply
    SO->>SO: saga COMPLETED
    SO--)K: media.processing.completed.v1

    K--)MQ: project into read model
    K--)NS: send notification

    C->>GW: GET /v1/media/{mediaId}
    GW->>MQ: proxy
    MQ-->>C: full media view
```
